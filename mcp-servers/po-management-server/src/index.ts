  ...
  async ({ run_id, created_by = 'system', created_by_user_id, line_items, container_plan, notes }) => {
    ...
    if (lineError && lineError.message.includes('expected_delivery_date')) {
      const fallbackRows = lineRows.map(row => {
        const next = { ...row };
        delete (next as any).expected_delivery_date;
        return next;
      });
...
server.tool(
  ...
  'Returns open PO quantities and nearest ETA by SKU, optionally broken down by supplier.',
  {
  ...
  async ({ skus, supplier_ids, statuses = ['draft', 'pending_approval', 'approved'] }) => {
    ...
    if (statuses && statuses.length > 0) query = query.in('purchase_orders.status', statuses);

    const { data, error } = await query;
    if (error) throw new Error(`get_open_commitments failed: ${error.message}`);

    const bySku: Record<string, { open_po_qty: number; next_open_po_eta: string | null }> = {};
    const bySupplier: Record<string, Record<string, { open_po_qty: number; next_open_po_eta: string | null }>> = {};
    ...
    for (const row of (data ?? []) as any[]) {
      ...
      if (!bySku[sku]) bySku[sku] = { open_po_qty: 0, next_open_po_eta: null };
      bySku[sku].open_po_qty += qty;
      if (eta && (!bySku[sku].next_open_po_eta || eta < bySku[sku].next_open_po_eta)) {
        bySku[sku].next_open_po_eta = eta;
      }

      if (!bySupplier[sku]) bySupplier[sku] = {};
      if (!bySupplier[sku][supplierId]) bySupplier[sku][supplierId] = { open_po_qty: 0, next_open_po_eta: null };
...
server.tool(
  ...
  async ({ po_number, new_status, reviewer, notes, line_item_overrides }) => {
    const { data: existingPO, error: existingPOError } = await supabase
      ...
      .single();
    if (existingPOError) throw new Error(`update_po_status lookup failed: ${existingPOError.message}`);
    ...
    if (logError) throw new Error(`update_po_status decision log failed: ${logError.message}`);
