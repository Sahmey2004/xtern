...
server.tool('ping', 'Health check', {}, async () => ({
  content: [{ type: 'text' as const, text: JSON.stringify({ server: 'erp-data-server', status: 'ok', timestamp: new Date().toISOString() }) }]
}));

server.tool(
  'get_products',
  'Returns products with MOQ, weight, volume, and price.',
  {
  ...
  async ({ skus, category }) => {
    ...
    if (error) throw new Error(`get_products failed: ${error.message}`);
    return { content: [{ type: 'text' as const, text: JSON.stringify({ products: data, count: data?.length ?? 0 }) }] };
  }
);

server.tool(
  'get_inventory',
  'Returns current inventory positions. Optionally filter to SKUs below reorder point.',
  {
    skus: z.array(z.string()).optional().describe('Filter to specific SKUs'),
  ...
  async ({ skus, below_reorder_only }) => {
    ...
    if (error) throw new Error(`get_inventory failed: ${error.message}`);
    let result = data ?? [];
    if (below_reorder_only) {
      result = result.filter((r: any) => (r.current_stock + r.in_transit) <= r.reorder_point);
    }
    return { content: [{ type: 'text' as const, text: JSON.stringify({ inventory: result, count: result.length }) }] };
  }
);

server.tool(
  'get_forecasts',
  'Returns demand forecasts for a planning horizon. Defaults to next 3 months.',
  {
    skus: z.array(z.string()).optional().describe('Filter to specific SKUs'),
  ...
  async ({ skus, months_ahead = 3, include_historical = false }) => {
    ...
    if (error) throw new Error(`get_forecasts failed: ${error.message}`);
    const summaryMap: Record<string, { sku: string; total_forecast: number; months: number }> = {};
    for (const row of (data ?? [])) {
      if (!summaryMap[row.sku]) summaryMap[row.sku] = { sku: row.sku, total_forecast: 0, months: 0 };
